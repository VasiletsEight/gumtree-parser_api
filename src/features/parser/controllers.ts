...
import {ParserAdvertisementPost, ParserCategoryPost, ParserFilePost, UserData} from "./type";
...
const postCategoryController = (request: FastifyRequest<ParserCategoryPost>, reply: FastifyReply) => {
    services.postCategoryService(request.body)
        .then((data: string[]) => reply.status(200).send({statusCode: 200, data: {paths: data}}))
...
export default {postCategoryController, postAdvertisementController, postCreateFileController};